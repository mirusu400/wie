//! Filesystem implementation copied from wie_cli/src/filesystem.rs.
//! Persistent storage under `<ProjectDirs::data_dir>/<aid>/fs/<path>`.
use std::{
    fs::{self, OpenOptions},
...
impl LibretroFilesystem {
    /// Build with an explicit data root, e.g. `<RA system>/wie/`.
    pub fn with_base(base_path: PathBuf) -> Self {
        Self { base_path }
    }

    /// Fallback: `<ProjectDirs::data_dir>/`. Used when the frontend doesn't
    /// expose a system directory.
    pub fn project_dirs_fallback() -> Self {
